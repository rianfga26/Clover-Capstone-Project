    ...
    <div class="flex flex-wrap items-center gap-5 md:gap-3 border rounded shadow-lg px-4 py-4">
        ...
        <div class="flex justify-between items-center gap-2 w-full">
            ...
            <div class="flex items-center gap-1">
                ...
                <p class="font-medium text-[10px] font-['Poppins'] md:text-base lg:text-sm">
                {{ $schedule->tgl_awal->format('d F Y') }} - {{ $schedule->tgl_akhir->format('d F Y') }} | 20:24 WIB
                </p>
            </div>
        </div>
        ...
        <div class="flex justify-between flex-wrap">
            ...
            <div class="flex flex-wrap gap-2 mb-2 sm:my-4 md:w-1/2 md:justify-end">
                ...
                <p class="font-['Poppins'] font-normal text-sm text-[#5B5858] md:text-base">{{ \Carbon\Carbon::parse($schedule->tgl_awal)->locale('id')->isoFormat('dddd') }},  {{ $schedule->tgl_awal->format('d F Y') }}</p>
                <p class="font-['Poppins'] font-normal text-sm text-[#5B5858] md:text-base">{{ \Carbon\Carbon::parse($schedule->tgl_awal)->locale('id')->isoFormat('HH:mm ') }} wib</p>
            </div>
            <div class="flex flex-wrap items-center gap-2 mb-2 sm:my-4 md:w-1/2">
