    ...
    <div class="container" id="fourth-section">
        <div class="w-full h-full pb-10 text-center">
            ...
            @forelse ($dokumentasimasters as $dokumentasimaster)
                <a href="{{ route('dokumentasi.detail', ['judul' => $dokumentasimaster->nama]) }}" class="relative flex justify-center items-center cursor-pointer border-4 border-gray-300 rounded sm:w-3/4 sm:w-auto">
                    @if ($dokumentasimaster->image)
                    <img src="{{ asset('storage/' . $dokumentasimaster->image) }}" alt="" class="w-full sm:w-[260px] lg:w-[340px]">
                    @else
                     No Image
                    @endif
                    <div class="absolute inset-0 bg-black opacity-40"></div>
                    <div class="absolute m-auto left-0 right-0">
                        <h5 class="mb-2 text-white font-['Poppins'] font-semibold text-2xl sm:text-[25px] lg:text-[40px] ">{{ $dokumentasimaster->nama }}</h5>
                        <p class="text-white font-['Poppins'] font-normal text-sm">{{ $dokumentasimaster->deskripsi }}</p>
                    </div>
                </a>
                @empty
                @endforelse  
